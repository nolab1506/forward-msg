...
def show_main_menu(chat_id, msg_id=None):
    ...
    text = (
        ...
        f"  {pipeline_status}  NODE_ID: VPS-SG-04\n"
        f"  ───────────────────────────────────────\n\n"
        f"  ● SYSTEM METRICS\n"
        f"  ───────────────────────────────────────\n"
        f"  Channels Connected  │  {target_count_num} Active\n"
        f"  Data Packets Synced │  {total_fwd:,} Messages\n"
        f"  Uptime Verification │  99.9% Stable\n"
        f"  Encryption Protocol │  TLS 1.3\n\n"
        f"  ● LIVE PIPELINE\n"
        f"  ───────────────────────────────────────\n"
        f"  Inbound  : {source_ch}\n"
        f"  Filter   : Custom Regex (Anti-Link)\n"
        f"  Outbound : {outbound_display}\n\n"
        f"  ───────────────────────────────────────\n"
