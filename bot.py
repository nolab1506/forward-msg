...
def my_channels(call):
    ...
    try:
        ...
        for ch in channels[:50]:
            ...
            # BUG FIX: Telethon returns positive peer IDs; prepend -100 only if needed
            ch_id = ch['id']
            if ch_id > 0:
                ch_id = int(f"-100{ch_id}")
            # If already negative (shouldn't happen with Telethon but guard anyway)
            elif not str(ch_id).startswith('-100'):
                ch_id = int(f"-100{abs(ch_id)}")
