...
def buy_plan(call):
    ...
    markup.row(types.InlineKeyboardButton(
        '✅ Payment করেছি', callback_data=f'paid_{plan_key}'
    ))
...
def approve_payment(call):
    ...
    bot.send_message(
        ...
        f"✅ *Payment Approved!*\n\n"
        f"Plan: *{plan['name']}*\n"
        f"Started: *{now.strftime('%Y-%m-%d')}*\n"
        f"Expires: *{expires[:10]}*\n\n"
        f"/start দিয়ে setup করুন! 🚀",
        parse_mode='Markdown'
    )
    bot.answer_callback_query(call.id, f"✅ Approved!")
...
def my_account(call):
    ...
    days_left = 'N/A'
    if user and user[4]:
        diff = datetime.fromisoformat(user[4]) - datetime.now()
        days_left = f"{max(0, diff.days)} দিন" if diff.days >= 0 else "Expired"
...
def handle_post(msg):
    ...
    c.execute(
        ...
        'f.text_replace, f.auto_delete_mins, u.plan '
    ...
    for u in users:
        (user_id, targets_str, watermark, silent,
         ...
         text_replace, auto_del, plan_key) = u
        ...
        for ch in targets:
            try:
                ...
                if msg.text and a_text:
                    ...
                    cap = apply_replace(msg.caption or '', text_replace if plan.get('text_replace') else '') + wm
...
def handle_edit(msg):
    ...
    c = conn.cursor()
    c.execute(
        'SELECT mm.channel_id, mm.sent_msg_id, f.watermark, f.text_replace, mm.user_id, u.plan '
        'FROM msg_map mm '
        'JOIN filters f ON mm.user_id = f.user_id '
    ...
    for row in rows:
        channel_id, sent_msg_id, watermark, text_replace, user_id, plan_key = row
        plan = PLANS.get(plan_key, {})
        wm = f"\n\n{watermark}" if (watermark and plan.get('watermark')) else ''
        ...
        try:
            if msg.text:
                ...
                cap = apply_replace(msg.caption, text_replace if plan.get('text_replace') else '') + wm
