...
def show_connect_screen(chat_id):
    ...
    text = (
        ...
        "🔐 *প্রথমে আপনার Telegram account connect করুন* 👇"
    )
    bot.send_message(chat_id, text, reply_markup=markup, parse_mode='Markdown')

def show_main_menu(chat_id, msg_id=None):
    sess = get_session(chat_id)
    conn_status = f"✅ Connected: *{sess[3]}*" if sess else "❌ Not Connected"
...
def connect_account(call):
    ...
    msg = bot.send_message(
        ...
        "আপনার phone number পাঠান:\n\n"
        "Format: `+8801XXXXXXXXX`\n\n"
        "⚠️ Country code সহ দিন",
        parse_mode='Markdown'
    )
