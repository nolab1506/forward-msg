...
def show_settings_menu(chat_id, msg_id):
    ...
    try:
        bot.edit_message_text(
            "⚙️ *Settings*\n\n"
            f"🔇 Silent Mode: *{'Enabled ✅' if f[3] else 'Disabled ❌'}*\n"
...
def _restart_listener(user_id):
    """Disconnect existing client — listener thread will auto-stop, then restart"""
    if user_id in active_clients:
        try:
            # Just disconnect; the listener thread will exit and active_listeners will be cleaned
...
# Track which users have active listeners
active_listeners = {}
...
def start_user_listener(user_id):
    ...
    async def _listen(my_loop):
        try:
            # Load or create client using this thread's loop
            sess = get_session(user_id)
            if not sess:
            ...
            f = get_filter(user_id)
            if not f or not f[1]:
                print(f"No source set for {user_id}, waiting...")
                # Keep client alive but no handler yet - wait for source to be set
    ...
    if user_id in active_listeners and active_listeners[user_id].is_alive():
        print(f"Listener already running for {user_id}, restarting...")
        # Signal old client to disconnect
        if user_id in active_clients:
            try:
                asyncio.run_coroutine_threadsafe(
                    active_clients[user_id].disconnect(), loop
                )
            except Exception:
                pass
            active_clients.pop(user_id, None)
        import time; time.sleep(1)  # give it time to stop
...
async def process_new_message(user_id, event):
    try:
        ...
        targets = f[2].split(',')
        watermark = f[3] or ''
        silent = bool(f[4])  # f[4] = silent_mode (was wrongly using f[3]=watermark)
        a_text = f[6]
        a_photo = f[7]
        a_video = f[8]
        a_doc = f[9]
