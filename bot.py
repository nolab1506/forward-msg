...
channel_page_cache = {}  # user_id -> {'channels': [...], 'page': 0, 'mode': 'src'|'tgt'}
PAGE_SIZE = 20
...
async def get_user_channels(user_id, filter_type=None):
    ...
    try:
        ...
        for d in dialogs:
            ...
            if d.is_channel and not d.is_group:
                ch_type = 'channel'
            elif d.is_group or (d.is_channel and d.is_group):
                try:
                    is_forum = getattr(entity, 'forum', False)
                except Exception:
                    is_forum = False
                ch_type = 'forum' if is_forum else 'group'
            else:
                continue
            ...
            is_mine = False
            try:
                is_mine = bool(getattr(entity, 'creator', False)) or \
                          getattr(entity, 'admin_rights', None) is not None
            except Exception:
                pass
            ...
            # Apply filter
            if filter_type == 'my_channel':
                if ch_type == 'channel' and is_mine:
                    channels.append(entry)
            elif filter_type == 'my_group':
                if ch_type == 'group' and is_mine:
                    channels.append(entry)
...
def confirm_disconnect(call):
    user_id = call.message.chat.id
    # BUG FIX: stop listener thread before disconnecting client
...
def my_channels(call):
    ...
    try:
        ...
        ch_icons = {'channel': '📢', 'group': '👥', 'forum': '💬'}
        for ch in channels[:50]:
            icon = ch_icons.get(ch['type'], '📌')
            mine = ' ★' if ch.get('is_mine') else ''
            ch_id = to_channel_id(ch['id'])  # ✅ FIXED
            text += f"{icon} *{ch['name']}*{mine}\n└ ID: `{ch_id}`\n\n"
...
def buy_plan(call):
    ...
    if plan_key == 'free':
        user = get_user(user_id)
        if user and user[5]:  # free_used is truthy (1)
...
def pick_source(call):
    ...
    if not sess:
        ...
        return
    bot.answer_callback_query(call.id)
    _show_source_filter_menu(user_id)

def _show_source_filter_menu(user_id):
    markup = types.InlineKeyboardMarkup()
    markup.row(
        types.InlineKeyboardButton('👥 Group',     callback_data='src_filter_group'),
        types.InlineKeyboardButton('📢 Channel',   callback_data='src_filter_channel'),
        types.InlineKeyboardButton('💬 Forum',     callback_data='src_filter_forum')
    )
    markup.row(
        types.InlineKeyboardButton('🤝 My Group',    callback_data='src_filter_my_group'),
        types.InlineKeyboardButton('📣 My Channel',  callback_data='src_filter_my_channel')
    )
    markup.row(types.InlineKeyboardButton('📋 Show All', callback_data='src_filter_all'))
    markup.row(types.InlineKeyboardButton('⬅️ Back', callback_data='go_home'))
    bot.send_message(
        user_id,
        "📥 *Select source type:*\n\nChoose a category to filter your channels 👇",
        reply_markup=markup, parse_mode='Markdown'
    )

def _send_src_page(user_id, page=0):
    cache = channel_page_cache.get(user_id)
    if not cache or cache['mode'] != 'src':
        bot.send_message(user_id, "❌ Session expired. Try again.")
        return
    ...
    for ch in page_channels:
        ch_id = to_channel_id(ch['id'])
        icon = ch_icons.get(ch['type'], '📌')
        mine = ' ★' if ch.get('is_mine') else ''
        markup.row(types.InlineKeyboardButton(
            f"{icon} {ch['name'][:28]}{mine}",
            callback_data=f"src_{ch_id}"
        ))

    nav_buttons = []
    if page > 0:
        nav_buttons.append(types.InlineKeyboardButton('⬅️ Prev', callback_data=f'src_page_{page - 1}'))
    if end < total:
        nav_buttons.append(types.InlineKeyboardButton('Next ➡️', callback_data=f'src_page_{page + 1}'))
    if nav_buttons:
        markup.row(*nav_buttons)

    markup.row(types.InlineKeyboardButton('🔙 Filter Again', callback_data='pick_source'))
    markup.row(types.InlineKeyboardButton('⬅️ Back', callback_data='go_home'))

    channel_page_cache[user_id]['page'] = page
    bot.send_message(
        user_id,
        f"📥 *Select source channel:*\n_{total} result(s) found — Page {page + 1}/{total_pages}_",
        reply_markup=markup, parse_mode='Markdown'
    )

@bot.callback_query_handler(func=lambda c: c.data.startswith('src_page_'))
def src_page_handler(call):
    user_id = call.message.chat.id
    page = int(call.data.replace('src_page_', ''))
    bot.answer_callback_query(call.id)
    _send_src_page(user_id, page)

@bot.callback_query_handler(func=lambda c: c.data.startswith('src_filter_'))
def src_filter_handler(call):
    user_id = call.message.chat.id
    filter_type = call.data.replace('src_filter_', '')
    bot.answer_callback_query(call.id)
    bot.send_message(user_id, "⏳ Loading channels...")
    try:
        ft = None if filter_type == 'all' else filter_type
        channels = run_async(get_user_channels(user_id, filter_type=ft))
        if not channels:
            bot.send_message(user_id, "❌ No channels found for this filter!\nTry another category.")
            _show_source_filter_menu(user_id)
            return
        channel_page_cache[user_id] = {'channels': channels, 'page': 0, 'mode': 'src'}
        _send_src_page(user_id, 0)
    except Exception as e:
        bot.send_message(user_id, f"❌ Error: {str(e)}")

@bot.callback_query_handler(func=lambda c: c.data.startswith('src_') and not c.data.startswith('src_page_') and not c.data.startswith('src_filter_'))
...
def pick_targets(call):
    ...
    if not sess:
        ...
        return
    bot.answer_callback_query(call.id)
    _show_target_filter_menu(user_id)

def _show_target_filter_menu(user_id):
    markup = types.InlineKeyboardMarkup()
    markup.row(
        types.InlineKeyboardButton('👥 Group',     callback_data='tgt_filter_group'),
        types.InlineKeyboardButton('📢 Channel',   callback_data='tgt_filter_channel'),
        types.InlineKeyboardButton('💬 Forum',     callback_data='tgt_filter_forum')
    )
    markup.row(
        types.InlineKeyboardButton('🤝 My Group',    callback_data='tgt_filter_my_group'),
        types.InlineKeyboardButton('📣 My Channel',  callback_data='tgt_filter_my_channel')
    )
    markup.row(types.InlineKeyboardButton('📋 Show All', callback_data='tgt_filter_all'))
    markup.row(types.InlineKeyboardButton('⬅️ Back', callback_data='go_home'))
    bot.send_message(
        user_id,
        "📤 *Select target type:*\n\nChoose a category to filter your channels 👇",
...
def _send_tgt_page(user_id, page=0):
    cache = channel_page_cache.get(user_id)
    if not cache or cache['mode'] != 'tgt':
        bot.send_message(user_id, "❌ Session expired. Try again.")
        return
    channels = cache['channels']
    total = len(channels)
    ...
    markup = types.InlineKeyboardMarkup()
    for ch in page_channels:
        ch_id = str(to_channel_id(ch['id']))
        icon = ch_icons.get(ch['type'], '📌')
        mine = ' ★' if ch.get('is_mine') else ''
        tick = ' ✅' if ch_id in selected else ''
        markup.row(types.InlineKeyboardButton(
            f"{icon} {ch['name'][:26]}{mine}{tick}",
            callback_data=f"tgt_{ch_id}"
        ))

    nav_buttons = []
    if page > 0:
        nav_buttons.append(types.InlineKeyboardButton('⬅️ Prev', callback_data=f'tgt_page_{page - 1}'))
    if end < total:
        nav_buttons.append(types.InlineKeyboardButton('Next ➡️', callback_data=f'tgt_page_{page + 1}'))
    if nav_buttons:
        markup.row(*nav_buttons)

    markup.row(types.InlineKeyboardButton(f'✅ Done ({len(selected)} selected)', callback_data='targets_done'))
    markup.row(types.InlineKeyboardButton('🔙 Filter Again', callback_data='pick_targets'))
    markup.row(types.InlineKeyboardButton('⬅️ Back', callback_data='go_home'))

    channel_page_cache[user_id]['page'] = page
    bot.send_message(
        user_id,
        f"📤 *Select target channels (multiple):*\n_{total} result(s) found — Page {page + 1}/{total_pages}_",
        reply_markup=markup, parse_mode='Markdown'
    )

@bot.callback_query_handler(func=lambda c: c.data.startswith('tgt_page_'))
def tgt_page_handler(call):
    user_id = call.message.chat.id
    page = int(call.data.replace('tgt_page_', ''))
    bot.answer_callback_query(call.id)
    _send_tgt_page(user_id, page)

@bot.callback_query_handler(func=lambda c: c.data.startswith('tgt_filter_'))
def tgt_filter_handler(call):
    user_id = call.message.chat.id
    filter_type = call.data.replace('tgt_filter_', '')
    bot.answer_callback_query(call.id)
    bot.send_message(user_id, "⏳ Loading channels...")
    try:
        ft = None if filter_type == 'all' else filter_type
        ...
        channel_page_cache[user_id] = {'channels': channels, 'page': 0, 'mode': 'tgt'}
        _send_tgt_page(user_id, 0)
    except Exception as e:
        bot.send_message(user_id, f"❌ Error: {str(e)}")

@bot.callback_query_handler(func=lambda c: c.data.startswith('tgt_') and not c.data.startswith('tgt_page_') and not c.data.startswith('tgt_filter_'))
...
def targets_done(call):
    ...
    # FIX: restart listener so new targets take effect immediately
    _restart_listener(user_id)
    show_main_menu(user_id)
...
def save_watermark(message):
    ...
    # FIX: restart listener so new watermark takes effect immediately
    _restart_listener(user_id)
...
def save_replace(message):
    ...
    # FIX: restart listener so new text replace takes effect immediately
    _restart_listener(user_id)
...
def stop_listener(user_id):
    ...
    # BUG FIX: remove from active_listeners FIRST so the restart loop knows to exit
...
def start_user_listener(user_id):
    ...
    def _run():
        # BUG FIX: auto-restart on crash so listener never permanently dies
        while True:
            ...
            try:
                ...
                active_listener_loops.pop(user_id, None)
                active_clients.pop(user_id, None)
            # Check if we should keep running or stop (e.g. user disconnected)
            if user_id not in active_listeners:
                print(f"[{user_id}] Listener stopped permanently")
                break
            print(f"[{user_id}] Listener crashed — restarting in 5s...")
            time.sleep(5)
    ...
    async def _listen(my_loop):
        ...
        try:
            ...
            if not f or not f[1]:
                ...
                # BUG FIX: only notify on first connection, not on every auto-restart
                try:
            ...
            # BUG FIX: Don't spam "Bot Connected" on every auto-restart
            # Only print to console, no Telegram notification here
            ...
            # FIX: sync deletes to all target channels
                ...
                for deleted_id in event.deleted_ids:
                    try:
                        ...
                        for channel_id, sent_msg_id in rows:
                            try:
                                await client.delete_messages(int(channel_id), sent_msg_id)
                                update_stats(user_id, 'total_deleted')
                            except Exception as e:
                                print(f"[{user_id}] Delete error in {channel_id}: {e}")
                    except Exception as e:
                        print(f"[{user_id}] on_delete error: {e}")
    ...
    active_listeners[user_id] = t  # BUG FIX: register BEFORE start so restart loop check works
    t.start()
            ...
            try:
                ...
                for msg in msgs:
                    if msg.media:
                        ...
                        captions.append(cap)
                        mime = getattr(msg.document, 'mime_type', '') or '' if msg.document else ''
                        is_image = msg.photo or mime.startswith('image/')
                        if is_image and watermark and plan.get('watermark'):
                            try:
                ...
                if sent_list:
                    ...
                    for msg_item, sent in zip(msgs, sent_list):
                        db.execute(
                            'INSERT OR REPLACE INTO msg_map VALUES (?, ?, ?, ?, ?)',
                            (msg_item.id, user_id, ch, sent.id, datetime.now().isoformat())
                        )
                    db.commit()
                    db.close()
        ...
        for ch in targets:
            try:
                ...
                if msg.media:
                    ...
                    if is_image and a_photo:
                        if watermark and plan.get('watermark'):
                            try:
                                img_bytes = await client.download_media(msg.media, bytes)
                                wm_bytes = add_watermark_to_image(img_bytes, watermark)
                                sent = await client.send_file(ch_id, io.BytesIO(wm_bytes), caption=cap, silent=silent, force_document=False)
                            except Exception:
                                sent = await client.send_file(ch_id, msg.media, caption=cap, silent=silent, force_document=False)
                        else:
                            try:
                                img_bytes = await client.download_media(msg.media, bytes)
                                sent = await client.send_file(ch_id, io.BytesIO(img_bytes), caption=cap, silent=silent, force_document=False)
                            except Exception:
                                sent = await client.send_file(ch_id, msg.media, caption=cap, silent=silent, force_document=False)
                    elif msg.video and a_video:
                        sent = await client.send_file(ch_id, msg.media, caption=cap, silent=silent)
                    elif msg.document and not is_image and a_doc:
                        sent = await client.send_file(ch_id, msg.media, caption=cap, silent=silent)
...
def load_all_sessions():
    ...
    for u in users:
        user_id = u[0]
        # BUG FIX: use start_user_listener which properly registers in active_listeners
        threading.Thread(
            target=start_user_listener,
            args=(user_id,),
            daemon=True
...
# BUG FIX: 409 Conflict — if another instance is running, Telegram will throw 409.
# Use skip_pending=True to clear old webhook/getUpdates conflicts on startup.
while True:
    try:
        ...
        if '409' in err or 'Conflict' in err:
            print(f"[BOT] 409 Conflict detected — another instance is running. Waiting 10s...")
            time.sleep(10)
        else:
